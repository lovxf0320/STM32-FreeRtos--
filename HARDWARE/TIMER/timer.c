...
void TIM6_Configuration(void){
    ...
    TIM_ITConfig(TIM6,TIM_IT_Updata,ENABLE);                    //打开定时器中断
...
void TIM7_Configuration(void){
    ...
    TIM_TimeBaseStructure.TIM_Period=36;                        //计数3600,1毫秒  
    TIM_TimeBaseStructure.TIM_Prescaler=20;                     //20分频
    TIM_TimeBaseStructure.TIM_ClockDivision=0;                  //不滤波
    TIM_TimeBaseStructure.TIM_CounterMode=TIM_CounterMode_Up;   //向上计数模式
    TIM_TimeBaseInit(TIM7,&TIM_TimeBaseInitStruct);             //初始化
    TIM_ITConfig(TIM7,TIM_IT_Updata,ENABLE);                    //打开定时器中断
